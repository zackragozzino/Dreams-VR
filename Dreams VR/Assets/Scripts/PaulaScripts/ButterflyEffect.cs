...
public class ButterflyEffect : MonoBehaviour {
   ...
   void Update () {  
      ...
      Vector3 towardsCenter = new Vector3(this.player.transform.position.x, this.player.transform.position.y + 1.5f, this.player.transform.position.z) - gameObject.transform.position; 
   ...
   private void moveCrawlerInCircle(GameObject c, Vector3 towardsCenter) {
      Vector3 newDir;
      float r = Random.Range(-3.0f, 3.0f) * 2.0f;
   ...
   private void moveCrawler(GameObject c, Vector3 direction) {
      ...
      Vector3 translate = (direction + new Vector3(r, r*3.0f, r))* this.speed * Time.deltaTime;
      c.transform.Translate(translate, Space.World);
